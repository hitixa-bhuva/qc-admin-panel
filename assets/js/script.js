...
function rm_menu() {
  var e = document.querySelector(".pc-sidebar"),
    ...
    e =
      (e && e.classList.remove("mob-sidebar-active"),
      t && t.classList.remove("mob-sidebar-active"),
      document.querySelector(".pc-sidebar .pc-menu-overlay")),
    t = document.querySelector(".topbar .pc-menu-overlay");
...
function remove_overlay_menu() {
  var e = document.querySelector(".pc-sidebar"),
    t = document.querySelector(".topbar");
  e &&
    (e.classList.remove("pc-over-menu-active"),
    (e = e.querySelector(".pc-menu-overlay"))) &&
    e.remove(),
    t &&
      (t.classList.remove("mob-sidebar-active"),
      (e = t.querySelector(".pc-menu-overlay"))) &&
      e.remove();
}
document.addEventListener("DOMContentLoaded", menu_click),
  document.addEventListener("DOMContentLoaded", function () {
    feather.replace(),
      (!document.querySelector("html").hasAttribute("data-pc-layout") ||
        ("horizontal" ==
    ...
    var e = document.querySelector("#mobile-collapse"),
      e =
        ...
        document.querySelector(".announcement-scroll-block") &&
          new SimpleBar(document.querySelector(".announcement-scroll-block")),
        document.querySelector(".component-list-card .scroll-div") &&
          new SimpleBar(
            document.querySelector(".component-list-card .scroll-div")
          ),
...
function removeClassByPrefix(t, r) {
  t.classList.forEach((e) => {
    e.startsWith(r) && t.classList.remove(e);
  });
}
document.querySelectorAll(".auth-main.v2 .img-brand").forEach((e) => {
  e.setAttribute("src", "./assets/images/logo-white.svg");
});
let slideUp = (e, t = 0) => {
...
var slideToggle = (e, t = 0) =>
  ("0px" === window.getComputedStyle(e).height ? slideDown : slideUp)(e, t);
...
(function(){
  ...
  function closeAll() {
    document.querySelectorAll('.dropdown-menu.show').forEach(m => {
      ...
      const btn = m.parentElement && m.parentElement.querySelector('.dropdown-toggle');
      if (btn) btn.setAttribute('aria-expanded','false');
    });
  }

  // toggle menu for a particular button
  ...
  document.addEventListener('click', function(e){
    ...
    if (item && item.closest('.dropdown-menu')) {
      ...
      const btn = group && group.querySelector('.dropdown-toggle');
      // update button label (keeps other children if you want preserve icons, adjust as needed)
      if (btn) btn.textContent = item.textContent.trim();
      // optionally store selected value on button dataset
      if (btn) btn.dataset.value = item.dataset.value ?? item.textContent.trim();
      closeAll();
      return;
    }
  ...
  // optional: close menus on window blur
